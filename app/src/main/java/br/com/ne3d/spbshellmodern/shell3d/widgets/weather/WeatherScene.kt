...
class WeatherScene : WidgetScene {
    ...
    override fun prepare(context: WidgetSceneContext) {
        ...
        clouds=Array(4) { i -> graph.root.add(SceneNode("cloud-$i").apply { mesh=MeshFactory.uvSphere(12, 8, .20f); material=WidgetMaterial(color=0xFFE7EDF5.toInt(), alpha=.9f); local.x=-.65f+i*.42f; local.y=.38f-(i%2)*.16f; local.z=.12f+i*.04f }) }
        fog=Array(3) { i -> graph.root.add(SceneNode("fog-$i").apply { mesh=MeshFactory.plane(.16f); material=WidgetMaterial(color=0xFFB9C8D5.toInt(), alpha=.26f); local.x=-.7f+i*.65f; local.y=-.08f+i*.12f; local.z=.18f; local.scaleX=.48f }) }
